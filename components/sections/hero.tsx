  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center pt-24 pb-16 overflow-hidden bg-orange-50">
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-[0.05] select-none pointer-events-none">
        <h1 className="text-[20vw] font-black tracking-tighter text-orange-900">ROOSTER</h1>
      </div>

      <div className="relative max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 w-full z-10">
        
        {/* أيقونات التواصل الاجتماعي - تم تصحيح الموضع والظهور */}
        <div className={`absolute -top-12 flex gap-4 ${isRtl ? 'right-6' : 'left-6'} z-50`}>
            <Link href="#" className="w-10 h-10 flex items-center justify-center bg-white text-orange-700 rounded-full hover:bg-orange-600 hover:text-white transition-all shadow-md border border-orange-200">
              <Facebook size={18}/>
            </Link>
            <Link href="#" className="w-10 h-10 flex items-center justify-center bg-white text-orange-700 rounded-full hover:bg-orange-600 hover:text-white transition-all shadow-md border border-orange-200">
              <Instagram size={18}/>
        ...
        <div className={`grid lg:grid-cols-2 gap-12 lg:gap-20 items-center ${isRtl ? 'direction-rtl' : ''}`}>
          ...
          <div className={`space-y-8 ${isRtl ? 'text-right' : ''}`}>
            ...
            {/* الأزرار مع روابط التمرير للأقسام */}
            <div className={`flex flex-wrap gap-4 pt-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
              ...
              >
                <Link href="#contact">
                  <Phone className="ms-2 h-5 w-5 mr-2" />
                  Contact
                </Link>
              </Button>
            </div>
          ...
          {/* بطاقة السلالة باللون البرتقالي الفاتح */}
          <div className="relative flex items-center justify-center z-10">
            <div className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-orange-950/5 border border-orange-100/50 w-full max-w-sm text-center space-y-6 transform lg:rotate-2 hover:rotate-0 transition-all duration-500 active:scale-95 cursor-pointer">
              
              {/* الدائرة الخلفية للصورة */}
              <div className="relative w-48 h-48 mx-auto bg-orange-50 rounded-full p-2 border-2 border-dashed border-orange-200 overflow-hidden shadow-inner group">
                <Image
                  src="/images/breeds/B2.png" // تأكد من وجود ملف الصورة
